...
class SenderWindow(SenderTestBase):
    def test_basic(self):
        ...
        self.expectSegment(conn, ack=True, payload=b'1234')
    ...
    def test_repeat(self):
        ...
        for _ in range(reps):
            ...
            self.expectSegment(conn, ack=True, payload_size=recvwin)
            self.expectNoSegment(conn)
    ...
    def test_window_growth(self):
        ...
        self.expectSegment(conn, ack=True, payload=b'0123')
        conn.segment_received(TcpSegment(TcpHeader(ack=True, ackno=isn+5, win=5)))
        self.expectSegment(conn, ack=True, payload=b'45678')
        self.expectNoSegment(conn)
    ...
    def test_fin_not_in_window(self):
        ...
        conn.write(b'1234567')
        conn.shutdown_write()
        self.expectSegment(conn, ack=True, payload=b'1234567')
        self.expectNoSegment(conn)
        self.assertEqual(conn.state, TcpState.ESTABLISHED)
    ...
    def test_piggyback_fin(self):
        ...
        conn.write(b'1234567')
        conn.shutdown_write()
        self.expectSegment(conn, ack=True, payload=b'123')
        self.expectNoSegment(conn)
...
class SenderTransmit(SenderTestBase):
    def test_three_short_writes(self):
        ...
        conn.write(b'ab')
        self.expectSegment(conn, ack=True, payload=b'ab', seqno=isn+1)
        conn.write(b'cd')
        self.expectSegment(conn, ack=True, payload=b'cd',seqno=isn+3)
        conn.write(b'abcd')
        self.expectSegment(conn, ack=True, payload=b'abcd', seqno=isn+5)
    ...
    def test_short_writes_and_countinuos_acks(self):
        ...
        for i in range(n_rounds):
            ...
            conn.write(data)
            self.expectSegment(conn, ack=True, payload=data, seqno=isn+bytes_send+1)
    ...
    def test_window_filling(self):
        ...
        self.expectSegment(conn, ack=True, payload=b'67')
        self.expectNoSegment(conn)
    ...
    def test_immediate_write_respect_windoew(self):
        ...
        conn.write(b'01')
        self.assertEqual(conn.bytes_in_flight, 2)
        self.expectSegment(conn, ack=True, payload=b'01')
        self.expectNoSegment(conn)
        self.assertEqual(conn.next_seqno, isn+1+2)
        conn.write(b'23')
        self.assertEqual(conn.bytes_in_flight, 3)
        self.expectSegment(conn, ack=True, payload=b'2')
        self.expectNoSegment(conn)
        self.assertEqual(conn.next_seqno, isn+1+3)     
...
class SenderACK(SenderTestBase):
    def test_repeat_ACK(self):
        ...
        conn.segment_received(TcpSegment(TcpHeader(ack=True, ackno=isn+1, win=3)))  
        conn.write(b'a')
        self.expectSegment(conn, ack=True, payload=b'a')
        self.expectNoSegment(conn)
        conn.segment_received(TcpSegment(TcpHeader(ack=True, ackno=isn+1, win=3)))  
        self.expectNoSegment(conn)
    ...
    def test_old_ACK(self):
        ...
        conn.write(b'b')
        self.expectSegment(conn, ack=True, payload=b'b')
        self.expectNoSegment(conn)
        conn.segment_received(TcpSegment(TcpHeader(ack=True, ackno=isn+1, win=3))) 
        self.expectNoSegment(conn)
